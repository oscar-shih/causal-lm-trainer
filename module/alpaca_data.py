...
def get_train_dataset(training_args, tokenizer, model_config):
    # Load dataset
    from datasets import load_dataset
    dataset = load_dataset("voidful/speech-alpaca-gpt4-unit")
    train_dataset = dataset['train'].filter(lambda x: x['hubert_layer6_code100_input_code'] != ''
                                            or x['hubert_layer6_code100_output_audio'] != '')
    # valid_dataset = dataset['dev'].filter(lambda x: x['hubert_100_question_unit'] != '')

    def process_data_to_model_inputs(item):
        input_sent = convert_vtok(item['hubert_layer6_code100_input_code'])
        output_sent = convert_vtok(item['hubert_layer6_code100_output_audio'])

        input_sent_tokens = tokenizer.encode(input_sent, return_tensors='pt',
                                                    add_special_tokens=False).to('cuda')
        label_sent_tokens = tokenizer.encode(output_sent, return_tensors='pt',
                                                    add_special_tokens=False).to('cuda')
